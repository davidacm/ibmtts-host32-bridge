...
pub fn handle_request(request: &[u8]) -> Vec<u8> {
    if request.len() < 2 {
        return pack_error("ERR:short request");
    }
    let id = u16::from_le_bytes([request[0], request[1]]);

    let ctx = RequestContext::new(request);

    // include the match block generated by build.rs
...
fn eci() -> Result<&'static EciApi, Vec<u8>> {
    ECI_API.get().ok_or_else(|| pack_error("ERR:no lib"))
}
...
pub fn pack_error(s: &str) -> Vec<u8> {
    let bytes = s.as_bytes();
    let mut out = Vec::with_capacity(1 + bytes.len());
    out.push(ResponseType::Error as u8);
    out.extend_from_slice(bytes);
    out
}
