...
fn main() {
    if env::var("CARGO_CFG_TARGET_OS").unwrap() == "windows" {
        ...
        res.set(
            "FileDescription",
            "32-bit host using Named Pipes and shared memory for IBM TTS (eci)",
        );
        res.set("ProductName", "IBM TTS host 32 Bridge");
        res.set("OriginalFilename", "ibmtts_host32.dll");
        res.set(
            "LegalCopyright",
            "Copyright © 2026 David CM. Licensed under GNU GPL v2.",
        );
        ...
        res.set(
            "ProjectURL",
            "https://github.com/davidacm/ibmtts-host32-bridge",
        );
    ...
    let api_files = vec!["src/worker.rs"];
    ...
    for file_path in api_files {
        ...
        let content = fs::read_to_string(file_path).expect("Could not read API file");
